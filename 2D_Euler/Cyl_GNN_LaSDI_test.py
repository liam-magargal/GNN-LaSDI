...
for sol in range(nSol):
    ...
    if sol in testSolsToDucros and N_lat in latDimToDucros:
        np.save('Cyl_solutions/Cyl_GNN_LaSDI_'+str(N_lat) + '_testSol_'+str(sol), x_hat_opInf)
    
    errors[sol] = np.linalg.norm(x_hist_approx[:,:]-x_hist_in_test[sol,:,:],'fro')/np.linalg.norm(x_hist_in_test[sol,:,:],'fro')
    

np.save('Cyl_solutions/Cyl_GNN_LaSDI_'+str(N_lat)+'_errors_',errors)
np.save('Cyl_solutions/Cyl_GNN_LaSDI_'+str(N_lat)+'_lowDimSolTime_',lowDimSolTime/numRep)
np.save('Cyl_solutions/Cyl_GNN_LaSDI_'+str(N_lat)+'_decodeTime_',decodeTime/numRep)
np.save('Cyl_solutions/Cyl_GNN_LaSDI_'+str(N_lat)+'_encodeTime_',encodeTime/numRep)
np.save('Cyl_solutions/Cyl_GNN_LaSDI_'+str(N_lat)+'_totalTime_',totalTime/numRep)

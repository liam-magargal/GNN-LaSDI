...
def getVelocity(edge_data, Mass_c, Momx_c, Momy_c, Energy_c, gamma, N, cell_centroids, cell_volumes, dt, u_1):
    ...
    for i in range(n_edges):
        ...
        if cell1==-1: #
            ...
            # top or bottom boundary (outflow boundary)
            rho_c1 = rho_c[cell0]
            vx_c1 = vx_c[cell0]
            vy_c1 = vy_c[cell0]
            P_c1 = P_c[cell0]
        ...
        epsilon = 1e-12*1. #1.0 is the reference velocity in this case (its a rough estimation of the maximum initial velocity) # epsilon = 1e-10
...
def updateState(edge_data, Mass_n, Momx_n, Momy_n, Energy_n, Mass_c, Momx_c, Momy_c, Energy_c, gamma, N, cell_centroids, cell_volumes, dt, u_1):
    # dont use parallel, just stick to njit
...
if __name__=="__main__":
    ...
    d0d1 = np.load(mesh_file + 'd0d1.npy') ##
    ...
    for t_step in range(501):
        ...
        x_hist_out[:N,t_step] = Mass_c
        x_hist_out[N:2*N,t_step] = Momx_c
        x_hist_out[2*N:3*N,t_step] = Momy_c
        x_hist_out[3*N:,t_step] = Energy_c

    t_end = time.time()
    t2 = time.time()
    solTime = t2-t1
    np.save('Cyl_getTimes/FOM_initTime_sol'+str(sys.argv[1])+'_rep_'+str(sys.argv[2]),initTime)
    np.save('Cyl_getTimes/FOM_solTime_sol'+str(sys.argv[1])+'_rep_'+str(sys.argv[2]),solTime)

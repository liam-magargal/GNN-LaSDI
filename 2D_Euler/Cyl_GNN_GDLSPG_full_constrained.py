...
def getGDLSPGsolution(x_init,N,edge_index,model,e1, e2, e3, e4, e5, u1, u2, u3, u4, pos1, pos2, pos3, pos4, pos5, ai_54, ai_43, ai_32, ai_21):
    ...
    for t_step in range(501):
        ...
        while True: 
            ...
            if iter_count==10 or iter_count==20:
               alpha = .1*alpha
...
if __name__=="__main__":
    ...
    model = architecture()
    model = torch.load('Cyl_GNN_lat_'+str(lat_dim) + '_500TS_constrained').cpu()
    ...
    for i in range(10):
        ...
        print('lat: ', lat_dim, ', sol: ', i)
        x_hist_in = np.load('Cyl_x_hist_500TS_test_'+str(1050+100*i)+'.npy')
        u_1 = u_all[i]
        ...
        for rep in range(numRep):
            ...
            te = time.time()
            totalTime[i] += te-ts
        if i in testSolsToDucros and lat_dim in latDimToDucros:
            np.save('Cyl_solutions/Cyl_GD_LSPG_'+str(lat_dim) + '_testSol_'+str(i), x_hat_hist.detach().cpu().numpy())
    ...
    np.save('Cyl_solutions/Cyl_GDLSPG_'+str(lat_dim)+'_errors_',errors)
    np.save('Cyl_solutions/Cyl_GDLSPG_'+str(lat_dim)+'_lowDimSolTime',lowDimSolTime/numRep)
    np.save('Cyl_solutions/Cyl_GDLSPG_'+str(lat_dim)+'_decodeTime',decodeTime/numRep)
    np.save('Cyl_solutions/Cyl_GDLSPG_'+str(lat_dim)+'_encodeTime',encodeTime/numRep)
    np.save('Cyl_solutions/Cyl_GDLSPG_'+str(lat_dim)+'_totalTime',totalTime/numRep)
